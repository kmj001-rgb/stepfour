// StepScraper: Enhanced Background Script with Dedicated Window Support
// Based on analysis of multiple Chrome extensions for robust image downloading

console.log('StepScraper: Background script initialized');
...
async function createDashboardWindow() {
    try {
        ...
        scrapingState.dashboardWindowId = window.id;
        logEvent('Dashboard window opened', 'info');
        return window;
    } catch (error) {
        logEvent(`Error creating dashboard window: ${error.message}`, 'error');
        throw error;
    }
}

async function focusDashboardWindow() {
    if (scrapingState.dashboardWindowId) {
        try {
            await chrome.windows.update(scrapingState.dashboardWindowId, { focused: true });
        } catch (error) {
            // Window might be closed, create a new one
...
async function processImageDownloads(thumbnails, destinations, settings) {
    ...
    const siteProfile = Utils.getSiteProfile(settings.currentUrl || '');
    let downloadFolder = settings.downloadFolder || 'StepScraper';
...
function updateProgress() {
    ...
    // Send to dashboard window if open
    if (scrapingState.dashboardWindowId) {
...
function logEvent(message, type = 'info', data = null) {
    ...
    console.log(`[StepScraper] ${type.toUpperCase()}: ${message}`, data);
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    switch (message.action) {
        ...
        case 'startScraping':
            handleStartScraping(message.settings, sender.tab);
            sendResponse({ success: true });
            break;
        ...
        case 'getSiteProfile':
            const profile = Utils.getSiteProfile(sender.tab?.url || '');
...
async function handleExportData(format = 'json') {
    try {
        ...
        if (format === 'json') {
            ...
            filename = `stepscraper_report_${new Date().toISOString().split('T')[0]}.json`;
        } else if (format === 'csv') {
            content = generateCSV(data);
            filename = `stepscraper_report_${new Date().toISOString().split('T')[0]}.csv`;
        }
...
chrome.windows.onRemoved.addListener((windowId) => {
    if (windowId === scrapingState.dashboardWindowId) {
        scrapingState.dashboardWindowId = null;
        logEvent('Dashboard window closed', 'info');
    }
});
...
chrome.runtime.onInstalled.addListener((details) => {
    if (details.reason === 'install') {
        logEvent('StepScraper installed successfully', 'success');
...
// Initialize
logEvent('StepScraper background script ready', 'info');