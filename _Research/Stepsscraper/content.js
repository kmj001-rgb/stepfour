// StepScraper: Enhanced Content Script with Advanced Pagination & Scraping
// Based on analysis of multiple Chrome extensions for robust gallery scraping

console.log('StepScraper: Content script loaded on', window.location.href);
...
async function extractPageData() {
    ...
    for (const img of images) {
        if (!img.src || !Utils.isValidImageUrl(img.src)) continue;
        ...
        if (!destinationUrl) {
            for (const selector of config.linkSelectors) {
                const link = img.closest(selector);
                if (link && link.href) {
...
async function scrapeAllPages(settings) {
    ...
    try {
        while (shouldScrapeContinue && pageNumber <= maxPages) {
            ...
            // Wait for navigation
            await Utils.wait(settings.pageWaitTime || 2000);
...
// Initialize
sendStatusUpdate('StepScraper content script ready 🚀');