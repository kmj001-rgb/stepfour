...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    try {
        ...
        if (request.action === 'startScrapePage' || request.action === 'startScrapeAllPages') {
            console.log('[Gallery Scraper] Starting async scraping operation');
            
            // Send immediate response to prevent timeout
            sendResponse({ success: true });
            
            // Check if we need to use injected script
            if (useInjectedScript) {
                console.log('[Gallery Scraper] Using injected script for scraping');
                ...
                // Use regular content script scraping
                (async () => {
                    try {
                        console.log('[Gallery Scraper] Async scraping started');
                        ...
                        console.error('[Gallery Scraper] Scraping error:', error);
                        ...
                        try {
                            ...
                            console.error('[Gallery Scraper] Failed to send error to background:', sendError);
                        }
                    }
                })();
            }
...
console.log('[Gallery Scraper] Content script initialization started');
sendStatusUpdate('🔧 Gallery Scraper Pro content script loaded');

// Check if we can inject the script directly
let useInjectedScript = false;

// Enhanced CSP detection and script injection
function detectCSPAndInject() {
    console.log('[Gallery Scraper] Content script verification - DOM ready:', document.readyState);
    console.log('[Gallery Scraper] Content script verification - URL:', window.location.href);
    
    // Check for CSP meta tag
    const metaCSP = document.querySelector('meta[http-equiv="Content-Security-Policy"]');
    if (metaCSP) {
        console.log('[Gallery Scraper] ⚠️ CSP detected:', metaCSP.content.substring(0, 100) + '...');
        ...
        if (metaCSP.content.includes('script-src') && !metaCSP.content.includes('chrome-extension')) {
            console.log('[Gallery Scraper] ❌ CSP blocks chrome-extension scripts');
    ...
    try {
        ...
        testElement.id = 'gallery-scraper-verification';
        document.body.appendChild(testElement);
        
        if (document.getElementById('gallery-scraper-verification')) {
            console.log('[Gallery Scraper] ✅ Content script DOM manipulation working');
            document.body.removeChild(testElement);
        } else {
            console.log('[Gallery Scraper] ❌ Content script DOM manipulation failed');
            useInjectedScript = true;
        }
    } catch (error) {
        console.log('[Gallery Scraper] ❌ Content script verification failed:', error.message);
        useInjectedScript = true;
    }
    
    // Additional check for specific problematic sites
    const problematicSites = ['imago-images.com', 'imago-images.de'];
    const currentDomain = window.location.hostname;
    if (problematicSites.some(site => currentDomain.includes(site))) {
        console.log('[Gallery Scraper] 🎯 Detected known CSP-restricted site:', currentDomain);
        useInjectedScript = true;
    }
    
    // If we need to use injected script, inject it now
    if (useInjectedScript) {
        console.log('[Gallery Scraper] Using injected script due to CSP restrictions');
        injectScript();
    } else {
        console.log('[Gallery Scraper] ✅ Using direct content script mode');
    }
}

// Verify content script is working
setTimeout(detectCSPAndInject, 1000);
...
function injectScript() {
    try {
        ...
        script.onload = function() {
            console.log('[Gallery Scraper] Injected script loaded successfully');
            sendStatusUpdate('🔧 Injected script loaded - CSP bypassed');
        };
        script.onerror = function() {
            console.log('[Gallery Scraper] Failed to load injected script');
            sendStatusUpdate('❌ Failed to load injected script', 'error');
        };
        document.head.appendChild(script);
    } catch (error) {
        console.log('[Gallery Scraper] Error injecting script:', error.message);
        sendStatusUpdate('❌ Error injecting script', 'error');
    }
}
...
window.addEventListener('message', function(event) {
    ...
    if (event.data.type === 'GALLERY_SCRAPER_RESPONSE') {
        console.log('[Gallery Scraper] Received response from injected script:', event.data);
        
        // Forward the response to background script
        if (event.data.success && event.data.data) {
            chrome.runtime.sendMessage({
                action: 'processScrapedData',
                data: event.data.data.thumbnails,
                failedExtractions: event.data.data.failedExtractions,
                currentPageUrl: event.data.data.currentPageUrl,
                isIntermediate: event.data.data.isIntermediate
            });
        }
    ...
    } else if (event.data.type === 'GALLERY_SCRAPER_STATUS') {
        console.log('[Gallery Scraper] Status from injected script:', event.data.message);
        sendStatusUpdate(event.data.message, event.data.type);
    }
});
