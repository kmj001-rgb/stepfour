...
function isValidImageUrl(url) {
  if (!url) return false;
  // Exclude SVGs and SVG data URLs
  if (url.startsWith('data:image/svg+xml') || url.endsWith('.svg')) return false;
  try {
    let parsedUrl = new URL(url);
    let pathname = parsedUrl.pathname;
    let lastSegment = pathname.split('/').pop();
    // If there's no dot, assume it's a valid image URL
    if (!lastSegment.includes('.')) return true;
    const extension = lastSegment.split('.').pop().split('?')[0].toLowerCase();
