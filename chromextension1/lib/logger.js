...
class Logger {
  ...
  _detectEnvironmentSync() {
    ...
    try {
      // First check if globalThis.PRODUCTION is set (from background.js)
      if (typeof globalThis !== 'undefined' && globalThis.PRODUCTION !== undefined) {
        this.isProduction = globalThis.PRODUCTION;
        this.minLogLevel = this.isProduction ? Logger.LOG_LEVELS.WARN : Logger.LOG_LEVELS.DEBUG;
      ...
      if (!isChromeExtension) {
        ...
        // In extension context - check for production URL pattern
        try {
          const url = chrome.runtime.getURL('');
          // Production: chrome-extension://[id]/ without localhost
          const isProductionUrl = url.startsWith('chrome-extension://') && !url.includes('localhost');
          this.isProduction = isProductionUrl;
          this.minLogLevel = this.isProduction ? Logger.LOG_LEVELS.WARN : Logger.LOG_LEVELS.DEBUG;
        } catch (e) {
          // If URL check fails, default to development for safety
          this.isProduction = false;
          this.minLogLevel = Logger.LOG_LEVELS.DEBUG;
        }
      }
